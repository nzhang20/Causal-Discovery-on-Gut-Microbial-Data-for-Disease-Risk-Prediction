...
def main(
    ...
    if save_cov_file== None:
        #SparCC
        os.system(f'python SparCC/Compute_SparCC.py  -n {name} -di {data_input} \
    ...
    #MakeBoostraps
    os.system(f'python SparCC/MakeBootstraps.py {data_input} -n {num_simulate_data} \
    ...
    for i in range(int(num_simulate_data)):
        ...
        os.system(f'python SparCC/Compute_SparCC.py  -n {name} -di {file_pvals1} \
             -m {method} -ni {n_iteractions} -xi {x_iteractions} \
             -th {threshold} -no {normalization} --log {log_transform} \
             -scor {corr_file1}')
    ...
    print("#"*100)
    os.system(f'python SparCC/PseudoPvals.py {save_corr_file} \
